...
echo "Building image..."
docker build -t ${local.image_name}:latest -t ${local.image_name}:${var.short_commit_sha} -f teams/people/organizations-management/rest-api/Dockerfile .
  ...
  template {
    ...
    containers {
      ...
      env {
        ...
        value = "teams/people/organizations-management/rest-api/entrypoint.sh"
      }
