...
locals {
  ...
  build_script = <<EOF
    ...
    echo 'Building app...'
    pnpm nx run people-skill-set-browser:build --prod --skip-nx-cache

    echo "Building image..."
    docker build -t ${local.image_name}:latest -t ${local.image_name}:${var.short_commit_sha} -f teams/people/skill-set/browser/Dockerfile . --no-cache
