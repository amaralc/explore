...
echo "Building image..."
docker build -t ${local.image_name}:latest -t ${local.image_name}:${var.short_commit_sha} -f teams/kernel/taxonomic-units/rest-api/Dockerfile .
  ...
  template {
    ...
    containers {
      ...
      env {
        ...
        value = "teams/kernel/taxonomic-units/rest-api/entrypoint.sh"
      }

      env {
        name  = "MONGODB_DATABASE_NAME"
        value = "kernel-taxonomic-units"
      }
...
resource "google_cloud_run_service_iam_member" "public" {
  ...
  # TODO: fix possible vulnerability, and allow only our proxy to access this service (PEER-750)
