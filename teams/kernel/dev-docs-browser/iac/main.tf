locals {
  ...
  build_script = <<EOF
echo 'Building app...'
pnpm nx run kernel-dev-docs-browser:build-docs --prod

echo "Building kernel-dev-docs-browser image..."
docker build -t ${local.image_name}:latest -t ${local.image_name}:${var.short_commit_sha} -f teams/kernel/dev-docs-browser/Dockerfile .
...
locals {
  service_domain = var.is_production_environment ? "${var.service_name}.${var.domain_name}" : "${var.service_name}.${var.environment_name}.${var.domain_name}"
}
