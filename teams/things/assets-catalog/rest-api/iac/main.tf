...
# TODO: try to find image from registry and use it as cache. If it doesn't exist, build it.

echo "Building image..."
docker build -t ${local.image_name}:latest -t ${local.image_name}:${var.short_commit_sha} -f teams/things/assets-catalog/rest-api/Dockerfile .
  ...
  template {
    ...
    containers {
      ...
      env {
        ...
        value = "teams/things/assets-catalog/rest-api/entrypoint.sh"
      }

      env {
        name  = "MONGODB_DATABASE_NAME"
        value = "things-assets-catalog"
      }
