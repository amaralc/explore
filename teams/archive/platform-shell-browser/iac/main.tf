...
module "instance" {
  ...
  build_command                    = var.is_production_environment ? "pnpm nx build core-platform-shell-browser --prod" : null # Use build-graph command to prevent building dependencies used for graphing purposes only
  output_directory                 = var.is_production_environment ? "dist/teams/core/platform-shell-browser/.next" : null     # Attention to the output of non-nextjs projects
