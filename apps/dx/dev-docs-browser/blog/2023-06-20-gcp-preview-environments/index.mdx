...
After making this change, we tested the hypothesis by running a new terraform apply, expecting that the "module.staging.data.external.bash" would still change, but
the "module.staging.module.postgresql_dbms.google_sql_database_instance.preview_environment[0]" would not. And that is exactly what happened.

While the result was what we expected, we still have to figure out how to pass the "environment_name" variable to the "environment" module dynamically.
