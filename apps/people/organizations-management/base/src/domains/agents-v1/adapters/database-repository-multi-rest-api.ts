...
export class MultiRestApiAgentsV1DatabaseRepository implements AgentsV1DatabaseRepository {
  constructor(
    private readonly multiInstitutionsV1BaseUrl: string,
    private readonly multiCentralsV1BaseUrl: string,
  ) {}
  ...
  private async getAllMultiCentralsV1(): Promise<Array<IMultiCentralV1Dto>> {
    const multiCentralsV1 = await fetch(this.multiCentralsV1BaseUrl).then(async (response) => {
      const json = (await response.json()) as { count: number; rows: Array<IMultiCentralV1Dto> };
      const rows = json.rows;
      return rows;
    });

    const uniqueIds = new Set(multiCentralsV1.map((item) => item.id));
    if (uniqueIds.size !== multiCentralsV1.length) {
      throw new Error('Duplicate IDs found in centrals data');
    }

    return multiCentralsV1;
  }
  ...
  public async listAll(): Promise<Array<IAgentV1Dto>> {
    const log: ILogMetadata = {
      steps: [],
      scope: {
        moduleName: MultiRestApiAgentsV1DatabaseRepository.name,
        methodName: 'listAll',
      },
    };
    ...
    try {
      ...
      log.steps.push({
        ...
        metadata: {
          ...
          agentsByEmail: agentsV1DtoList.reduce((acc, item) => {
