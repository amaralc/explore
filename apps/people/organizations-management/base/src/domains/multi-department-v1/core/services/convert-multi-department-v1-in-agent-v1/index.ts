...
import { IConvertMultiDepartmentV1InAgentV1InputDto } from './dto';

export class ConvertMultiDepartmentV1InAgentV1Service {
  static execute(inputDto: IConvertMultiDepartmentV1InAgentV1InputDto): IAgentV1Dto {
    const id = hashIntegerAndEntityNameIntoValidFirebaseUID(inputDto.id, 'MultiDepartmentV1');

    const agentV1 = new AgentV1Entity({
      id,
      type: 'ORGANIZATION',
      email: 'placeholder-' + id + '@email.com', // TODO: get actual institution emails
      nickname: stringToSlug(inputDto.nome + '-' + id),
