...
export class RestApiMultiCentralsV1DatabaseRepository implements MultiCentralsV1DatabaseRepository {
  constructor(private readonly multiCentralsV1BaseUrl: string) {}

  public async listAll(): Promise<Array<IMultiCentralV1Dto>> {
    const log: ILogMetadata = {
      steps: [],
      scope: {
        moduleName: RestApiMultiCentralsV1DatabaseRepository.name,
        methodName: 'listAll',
      },
    };
    ...
    try {
      ...
      const multiCentralsV1Response = (await fetch(this.multiCentralsV1BaseUrl).then((response) =>
        response.json(),
      )) as {
        count: number;
