...
echo "Building image..."
docker build -t ${local.image_name}:latest -t ${local.image_name}:${var.short_commit_sha} -f apps/kernel/taxonomic-units/rest-api/Dockerfile .
  ...
  template {
    ...
    containers {
      ...
      env {
        ...
        value = "apps/kernel/taxonomic-units/rest-api/entrypoint.sh"
      }
