...
echo "Building image..."
docker build -t ${local.image_name}:latest -t ${local.image_name}:${var.short_commit_sha} -f apps/things/assets-catalog/rest-api/Dockerfile .
  ...
  template {
    ...
    containers {
      ...
      env {
        ...
        value = "apps/things/assets-catalog/rest-api/entrypoint.sh"
      }
