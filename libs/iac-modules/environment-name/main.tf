...
locals {
  staging_branch_name = var.branch_name
}
...
resource "null_resource" "ignore_branch_name_changes" {
  triggers = {
    branch_name = data.external.parse_branch_name.result["environment_name"]
  }
...
output "instance" {
  value = null_resource.ignore_branch_name_changes.triggers["branch_name"]
}
